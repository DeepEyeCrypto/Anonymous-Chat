...
> **Date**: 2026-02-21  
> **Focus**: Advanced Stealth (P4), Quantum Hybrid E2EE (P5), and Multi-Device Sync (P6).
...
├── android/               # Kotlin/Compose app (multi-module Gradle)
├── rust/                  # Rust workspace (all native crates)
...
| :--- | :--- | :--- |
| `main` | Stable, released code. Every commit is deployable. | — |
| `dev` | **NEW** — Integration branch for v2.0 work | `main` via PR |
...
| **Staging** | N/A for Phase 1 (no server). | — | — |
...
### 3.1 Rust CI (`rust-ci.yml`) — On push/PR to `main` and `dev`
...
### 3.2 Android CI (`android.yml`) — On push/PR to `dev` (build check)

**NEW pipeline for `dev` branch PRs** — fast feedback without full release:
...
Steps:
  ...
  5. Build Android App (assembleDebug only — no native libs needed)
...
### 3.3 Android Release (`android-release.yml`) — On tag push `v*`
...
**`settings.gradle.kts`** — must include new modules:
...
| :--- | :--- | :--- |
| **GitHub Releases** | ✅ Active | Auto-created on tag push. APK downloadable. |
| **Direct sideload** | ✅ Active | Share APK file directly. Signed with debug keystore. |
| **F-Droid** | 🔜 Phase 2 | Requires reproducible builds + metadata YAML. |
| **Play Store** | 🔜 Phase 3 | Requires release keystore + Play Console account. |
...
**Phase 1 keeps debug keystore for release.** This is intentional — sideload-only distribution doesn't need Play Store signing. We'll upgrade when targeting stores.
...
| **User bug reports** | Manual — user shares Logcat output via GitHub Issues |
| **Crash detection** | `Thread.setDefaultUncaughtExceptionHandler` → log to file → show "Report" dialog on next launch |
...
□ Dev branch passes Rust CI (check + test + clippy)
□ Dev branch passes Android CI (assembleDebug + test)
□ CHANGELOG.md updated with new version entry
□ Version bumped in build.gradle.kts (versionName + versionCode)
□ PR from dev → main created with summary
□ PR reviewed and merged
□ Tag created on main: git tag -a v{version} -m "{summary}"
□ Tag pushed: git push origin v{version}
□ GitHub Actions builds APK and creates Release
□ Download APK from Release, install on test device, smoke test
□ Release published (draft → full release)
```

### 6.2 Smoke test (manual, per release)

```text
□ Fresh install on clean emulator/device
□ Onboarding completes
□ Identity generated and fingerprint displayed
□ All 4 tabs navigate correctly
□ Settings shows identity and core status
□ WIPE IDENTITY works (app resets to onboarding)
